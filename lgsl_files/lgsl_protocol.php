 ...
 ///   "wolfrtcw"      => "Wolfenstein: Return To Castle Wolfenstein",
  ...
  function lgsl_list_mods()
  {
    return array(
	  ...
	  "soccerjam"   => "Soccer Jam",
	  "paintball"   => "Paintball",
	  "hidehseek"   => "Hide N Seek",
	  "superhero"   => "SuperHero",
	  "fun"         => "Fun",
	  "furien"      => "Furien",
	  "other"       => "Other",
	  ...
	  "jailbreak"   => "JailBreak");
	return $lgsl_list_mods;  
  }
  
  function lgsl_list_halflife()
  {
    return array(
      "classic"    => "Classic",
	  "public"     => "Public",
	  ...
	  "capturetheflag"   => "Capture The Flag",
	  "jailbreak"  => "JailBreak");
	return $lgsl_list_halflife;  
  }
  
  function lgsl_list_csgo()
  {
    return array(
      "classic"     => "Classic",
	  "public"      => "Public",
	  "deathmatch"  => "Deathmatch",
	  "other"       => "Ostatné");
	return $lgsl_list_csgo;
  }
  
  function lgsl_list_source()
  {
    return array(
      "classic"     => "Classic",
	  "public"      => "Public",
	  ...
	  "onlymaps"    => "Only maps",
	  "jailbreak"   => "JailBreak");
	return $lgsl_list_source;  
  }
  
  function lgsl_list_minecraft()
  {
    return array(
	  "public"       => "Public",
	  "survival"     => "Survival",
	  ...
	  "skyblock"     => "SkyBlock",
	  "other"        => "Ostatné");
	return $lgsl_list_minecraft;  
  }
  
  function lgsl_list_samp()
  {
    return array(
	  "public"      => "Public",
	  "drift"       => "Drift",
	  ...
	  "gangwar"     => "Gang War",
	  "other"       => "Ostatné",
	  "crazyserver" => "Crazy Server",
      "reallife"    => "Realna zem");
	return $lgsl_list_samp;  
  }
  
  function lgsl_list_ts3()
  {
    return array(
      "other" => "Ostatné");
	return $lgsl_list_ts3;  
  }
...
//---------------------------------------------------------+
    ...
    elseif ($response_type == "D") // SOURCE AND HALF-LIFE 1 PLAYERS
    {
      ...
      while ($buffer)
      {
        $server['p'][$player_key]['pid']   = lgsl_cut_byte($buffer, 1);
...
//  SERVER SETTINGS
	...
	elseif ($server['b']['type'] == "minecraft") 
	{
		...
		if (isset($server['s']['name'])) { $server['s']['name'] = lgsl_parse_color($server['s']['name'], "minecraft"); } else { $server['s']['name'] = "--"; }
...
//------------------------------------------------------------------------------------------------------------+
  ...
  {
//---------------------------------------------------------+
//  REFERENCE: http://flstat.cryosphere.co.uk/global-list
...
//---------------------------------------------------------+
//  REFERENCE:
//  http://hazardaaclan.com/wiki/doku?id=aa3_server_query
//  http://aluigi.altervista.org/papers.htm#aa3authdec

    if (!function_exists('gzuncompress')) { return FALSE; } // REQUIRES http://www.net/zlib
...
//------------------------------------------------------------------------------------------------------------+
  ...
  {
//---------------------------------------------------------+
//  REFERENCE: http://www.cs2d.com/servers
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function lgsl_parse_color($string, $type)
  {
    switch($type)
    {
      ...
      case "minecraft":
			//$string = preg_replace("/[&\\xa7][0-9a-f]/", "", $string);
			//$string = preg_replace("/\xA7[0-9A-FK-OR]/i", "", $string);
        $string = preg_replace("/[&\xA7][0-9a-fA-Fk-oK-OrR]/", "", $string);
			//$string = preg_replace("/[&\xA7][0-9a-fk-or]/", "", $string);
      break;
    }
    return $string;
  }
