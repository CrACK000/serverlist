<?php
	...
	if(isset($id) && is_numeric($id))
	{
		...
		{
			$output = "<div class='text-center'><p>Novinka s tak�mto ID v na�ej datab�ze neexistuje, alebo bola vymazan� adminom k�m sa ti na��tal web.</p></div>";
			return;
		}
		$vyber = mysql_query( "SELECT * FROM `".$lgsl_config['db']['newstable']."` WHERE news_id='".$id."'");
		$data = mysql_fetch_assoc( $vyber );
		
		$output = "<h2>".$data['news_name']."</h2><h4>Pridan� d�a: ".Date("d.M.Y H:i:s",$data['news_date'])."</h4><hr />";
		...
		$output .= "<div class='fb-like' data-href='http://".$_SERVER['HTTP_HOST']."/novinky?viewid=".$id."' data-layout='button_count' data-action='like' data-show-faces='false' data-share='true'></div>";
		$output .= "<div class='right'><a href='http://".$_SERVER['HTTP_HOST']."' class='border-none'>Vr�ti� sa na index</a></div>";
	}
	else
	{
		$output = "<div class='text-center'><p>Zadal si chybn� ID novinky pre zobrazenie alebo in� chyba v odkaze.</p></div>";
	}
?>